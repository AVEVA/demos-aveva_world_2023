...
def threadManager(appsettings, thread_partition):
    ...
    omf_client = ADHOMFClient(
        ...
        thread_partition[1].get('ClientId'),
        thread_partition[1].get('ClientSecret'),
...
def processManager(appsettings, process_partition):
    thread_partitions = []
    for i in range(0, max_threads_per_process):
        thread_partitions.append(
            (process_partition[0][i::max_threads_per_process],
             process_partition[1][i % len(process_partition[1])]))

    threads = []
    for thread_partition in thread_partitions:
        thread = threading.Thread(
            target=threadManager, args=(appsettings, thread_partition))
...
def main():
    ...
    omf_client.verifySuccessfulResponse(
        response, 'Error creating containers')

    process_partitions = []
    for i in range(0, max_processes):
        process_partitions.append(
            (streams[i::max_processes], appsettings.get('Clients')[i::max_processes]))
